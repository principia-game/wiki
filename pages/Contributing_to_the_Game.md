...
## LLM usage policy
Submitting pull requests to the project that are "vibe-coded" or otherwise generated by LLMs/AI is not allowed, whether it be by autonomous AI agents or humans. You should, as a general rule, understand the code changes you are contributing and respect our wish for the project's development to remain human.

Do not use GitHub Copilot or any other AI to describe your changes, either in the PR description or after the PR is opened by invoking the AI. This typically just adds noise without giving any useful information, and you as the PR author should be better suited to describe the changes that have been made in a succinct manner.

Note that using LLMs to write issue descriptions is also not allowed as they typically just make it more verbose without adding any useful information. Additionally, using LLMs to write documentation, such as content for this Wiki [is not allowed either](/wiki/Contributing_to_the_Wiki#llm-usage-policy).

Running static analysis tools (cppcheck, sanitisers, fuzzers, etc...), correcting typos, and other things that may use automated tooling but not LLMs is still allowed and encouraged. Every contribution is welcome regardless of the size.
