...
It can also be used for keeping track of a value between 1-32, as an alternative to the [[Switch]] for more than 5 switch branches. Simply calculate the fraction for each position as `position / Max value` and check against a [[Jumper]] of that value with a [[cmp-e]]. See [32-bit Controllable Binary Counter](/level/8) for an example of this, which uses a condenser with a Max Value of 32.
